#!/bin/bash
oozie jobs -oozie http://localhost:11000/oozie -kill -filter user=root -jobtype bundle & oozie jobs -oozie http://localhost:11000/oozie -kill -filter user=root
 -jobtype coordinator & oozie jobs -oozie http://localhost:11000/oozie -kill -filter user=root
...
sqoop job --meta-connect jdbc:hsqldb:hsql://localhost:16000/sqoop  --create cdw_branch_import -- import -m1 --connect jdbc:mysql://localhost/CDW_SAPP
 ...
 LAST_UPDATED
 FROM
 CDW_SAPP_BRANCH WHERE $CONDITIONS' --append --incremental lastmodified --check-column LAST_UPDATED --last-value '0'
...
sqoop job --meta-connect jdbc:hsqldb:hsql://localhost:16000/sqoop --create cdw_customer_import -- import -m1 --connect jdbc:mysql://localhost/CDW_SAPP
 ...
 LAST_UPDATED
 FROM
 CDW_SAPP_CUSTOMER WHERE $CONDITIONS' --append --incremental lastmodified  --check-column LAST_UPDATED --last-value '0'
...
sqoop job --meta-connect jdbc:hsqldb:hsql://localhost:16000/sqoop --create cdw_credit_import -- import -m1 --connect jdbc:mysql://localhost/CDW_SAPP
 ...
 --append --incremental lastmodified --check-column LAST_UPDATED --last-value '0'
...
sqoop job --meta-connect jdbc:hsqldb:hsql://localhost:16000/sqoop --create cdw_time_import -- import -m1 --connect jdbc:mysql://localhost/CDW_SAPP --driver com.mysql.jdbc.Driver --query 'SELECT
 ...
 "Third" WHEN month BETWEEN 10 AND 12 THEN "Fourth" END, YEAR, Transaction_ID , LAST_UPDATED FROM CDW_SAPP_CREDITCARD WHERE $CONDITIONS'
 --append --incremental lastmodified --check-column LAST_UPDATED --last-value '0'
